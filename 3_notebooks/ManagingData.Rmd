...
The next section details how we would otherwise sample the dataset to look
for only the files of interest and avoid scraping the files with repeated or
...
#rootdir <- datadir
#files <- list.files(datadir, pattern = "tfl_1-[^.]+\\.xml")
...
```{r subset_given_day}
### Retrieve Vehicle Journeys that relate to a given day of timetabling ----
...
```{r weekday_category}
# Get the day of the week and whether its a Weekend or MondayToFriday
get_weekday_category <- function(date){
...
```{r subset_file_list}
weekday_vec <- get_weekday_category(timetable_date)
xpath.required_days <- sprintf(".//txc:VehicleJourneys/txc:VehicleJourney[descendant::txc:DaysOfWeek/txc:%s or descendant::txc:DaysOfWeek/txc:%s]", weekday_vec[1], weekday_vec[2])

XML::xmlParse(data.files[1]) %>%
  XML::xmlRoot() %>%
  XML::xpathApply(xpath.required_days, namespaces = namespace)


terminal_nodesets <- lapply(xpath.required_days, xml2::xml_find_all, x = xml2::read_xml(data.files[1]), ns = namespace)
```
